...
# Make default for newly opened shells
profiles=( ~/.local/share/konsole/*.profile )
profileCount=${#profiles[@]}

if [ $profileCount -eq 0 ]; then
	echo "Sorry, you have no profiles to edit"
elif [ $profileCount -gt 1 ]; then
	echo "Sorry, there are multiple profile files, unsure which to edit"
	echo "You may set this theme manually for future Konsole sessions"
	echo " by going to 'Settings -> Edit Current Profile -> Appearance -> Colorscheme generated by wal'"
else
	for f in ${profiles[@]}; do
		sed -i -e "s/ColorScheme=.*$/ColorScheme=AutoImport-$suffix/" "$f"
	done
fi
